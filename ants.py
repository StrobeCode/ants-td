...
class Ant(Insect):
    ...
    def buff(self):
        ...
        if not self.buffed:
            self.damage *= 2
            self.buffed = True
        if self.is_container and self.ant_contained is not None and not self.ant_contained.buffed:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if len(place.bees) and self.min_range <= transition <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        place = self.place
        for bee in place.bees[:]:  # This "[:]" is indispensable!!! It makes a copy of the original list.
